    ...
    if (extract(&a1_receive_data, raw_data))//反馈数据crc校验通过，同时将接收到的u8数据处理到结构体里
    {
			...
			switch(a1_receive_data.ID)
			{
				case 1:
					...
					if(Motor_A1.Flag_Init == 0)//记录此时的初始角度，以此为基准确定反馈角度和控制角度
					{
						Motor_A1.Ctrl_Data.Pos = Motor_A1.Rec_Data.Pos;
						Motor_A1.Init_Rad = Motor_A1.Rec_Data.Pos;
						Motor_A1.Flag_Init = 1;
					}
					Motor_A1.Real_Delta_Deg = (Motor_A1.Rec_Data.Pos-Motor_A1.Init_Rad)/(9.1f*RADIAN);//计算电机减速后的转子真实转动角度
					
					system_monitor.motor_A1_cnt++;
					break;
				default:
					break;
			}
...
{
	ctrl_data->Ctrl_Data.Pos = ctrl_data->Target_Delta_Deg*RADIAN*9.1f + ctrl_data->Init_Rad;
	
	uint8_t data[34];
	modify(&ctrl_data->Ctrl_Data,data);
	HAL_UART_Transmit_DMA(&huart6,data,34);
