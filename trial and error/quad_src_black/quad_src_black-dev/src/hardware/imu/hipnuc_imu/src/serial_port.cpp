	...
	class IMUPublisher : public rclcpp::Node
	{
		public:
			...
			IMUPublisher(const rclcpp::NodeOptions &options) : Node("IMU_publisher", options)	
			{
				...
				this->declare_parameter<std::string>("imu_topic", "/quad/test_estm_state");
				...
				// ✅ 修改：初始连接
				if (connect_serial() < 0) {
					RCLCPP_ERROR(this->get_logger(), "Failed to connect to IMU initially");
				}
				...
				read_timer_ = this->create_wall_timer(
					1ms,
					std::bind(&IMUPublisher::imu_read_timer_callback, this)
				);
				
				// ✅ 添加：连接监控定时器（100ms 检查一次）
				monitor_timer_ = this->create_wall_timer(
					100ms,
					std::bind(&IMUPublisher::monitor_connection, this)
				);
			}
		...
		private:
			...
			int connect_serial() {
				...
				RCLCPP_INFO(this->get_logger(), "Attempting to connect to %s...", serial_port.c_str());
				
				fd = open_ttyport(serial_port, baud_rate);
				
				if (fd < 0) {
					RCLCPP_WARN(this->get_logger(), "Failed to open serial port: %s", serial_port.c_str());
					is_connected_ = false;
					return -1;
				}
				...
				RCLCPP_INFO(this->get_logger(), "Successfully connected to IMU on %s", serial_port.c_str());
				return 0;
			}
			...
			void monitor_connection() {
				if (!is_connected_ || fd < 0) {
					...
					if ((now - last_reconnect_attempt_).seconds() >= RECONNECT_INTERVAL) {
				...
				for(int i = 0; i < n; i++)
				{
                    ...
                    if(rev)
                    {
                        ...
                        if (raw.hi91.tag == 0x91) {
                            ...
                            imu_msg.imu_world.ang_vel = {
                                -raw.hi91.gyr[0] * DEG_TO_RAD,
			...
			int open_ttyport(std::string tty_port, int baud)
			{
				...
				if (serial_port < 0)
				{
					...
					const std::string fallback_port = "/dev/ttyUSB1";
					RCLCPP_INFO(this->get_logger(), "Trying fallback port: %s", fallback_port.c_str());
					serial_port = open(fallback_port.c_str(), O_RDWR | O_NOCTTY | O_NONBLOCK);
					if (serial_port < 0)
					{
						perror("Error opening fallback serial port");
						return -1;
					}
					RCLCPP_INFO(this->get_logger(), "Using fallback port: %s", fallback_port.c_str());
				}
