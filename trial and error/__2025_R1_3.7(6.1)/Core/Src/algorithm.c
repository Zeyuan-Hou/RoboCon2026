...
void LESO_Order1(ST_LESO_1order *leso_1order, float y, float U0)
{
    ...
    if (leso_1order->U >= leso_1order->fpUMax)
        leso_1order->U = leso_1order->fpUMax;
    if (leso_1order->U <= -leso_1order->fpUMax)
        leso_1order->U = -leso_1order->fpUMax;
}
...
void LESO_Order2(ST_LESO_2order *leso_2order, float y, float U0)
{
    ...
    if (leso_2order->U >= leso_2order->fpUMax)
        leso_2order->U = leso_2order->fpUMax;
    if (leso_2order->U <= -leso_2order->fpUMax)
        leso_2order->U = -leso_2order->fpUMax;
}
/*-------------------------------------------------------------------------------------------------
...
float plan_of_time(float start_pos, float end_pos, float time)
{
    ...
    if (time >= 0.0f && time <= 1.0f)
        k = 6 * time * time * time * time * time - 15 * time * time * time * time + 10 * time * time * time;
    else if (time > 1.0f)
        k = 1;
...
void J60_motor_control(void)
{
    ...
    else
    {
        ...
        J60_Motor_Ctrl.torque_ = ClipFloat(5e-6 * J60_angle * J60_angle * J60_angle * J60_angle - 0.0005 * J60_angle * J60_angle * J60_angle + 0.013 * J60_angle * J60_angle + 0.0471 * J60_angle - 0.0079, 0, 4.9);
    }
}
