...
void parseDataPacket(const uint8_t Rx_Buf[32], ST_JS_VALUE *jsValue)
{
	...
	//	uart1_tx_buffer[3]=0;
	// 再给上板发按键值
	if (jsValue->usJsKey != pre_key && jsValue->usJsKey != 0)
	{
		uart1_tx_buffer[4] = jsValue->usJsKey; // 用于上下板通讯
		pre_key = jsValue->usJsKey;
	}
	else if (jsValue->usJsKey == 0)
	{
		uart1_tx_buffer[4] = pre_key; // 用于上下板通讯
	}
	...
	case 30:																 // 前进按钮
		...
		// 遥控控制竞技赛三区切换状态
		if (nav.nav_state == NAV_AREA_3 && (nav.auto_path.run_time >= 1000))
		{
			state3 += 1;
			flag_record = 1;
			nav.auto_path.run_time = 0;
		...
		// 遥控控制单项赛三区切换状态
		if (nav.nav_state == NAV_AREA_3_SINGLE && (nav.auto_path.run_time >= 1000))
		{
			state3_SINGLE += 1;
			flag_record = 1;
			nav.auto_path.run_time = 0;
