...
/*******************************************************************
函数名称：NormalizeAngle()
函数功能：将角度转换为[-180,180]°范围的等效角度（单位：度）
输入：    angle：目标角度(单位：度)
输出：    转换后的角度(单位：度)
备注：    逆时针为正，顺时针为负，支持任意大小的角度值转换
********************************************************************/
double NormalizeAngle(double angle) {
    // 第一步：对360取模，得到0~360度范围内的角度
    double normalized = fmod(angle, 360.0);
    
    // 处理负数取模的特殊情况（fmod返回值符号与被除数一致）
    if (normalized < 0) {
        normalized += 360.0;
    }
    
    // 第二步：将大于180度的角度转换到-180~0度区间
    if (normalized > 180.0) {
        normalized -= 360.0;
    }
...
/*******************************************************************
函数名称：GetMinTurnAngle()
函数功能：计算从起始角度转到目标角度的最小转角（含方向）
输入：    startAngle：起始角度(单位：度)
          targetAngle：目标角度(单位：度)
输出：    最小转角值(单位：度，范围[-180,180]，正值逆时针，负值顺时针)
备注：    依赖NormalizeAngle函数，确保角度先归一化再计算
********************************************************************/
double GetMinTurnAngle(double startAngle, double targetAngle) {
    // 先将两个角度归一化到[-180,180]范围
    double normStart = NormalizeAngle(startAngle);
    double normTarget = NormalizeAngle(targetAngle);
    
    // 计算原始角度差
    double angleDiff = normTarget - normStart;
    
    // 归一化角度差，得到最小转角（核心逻辑）
    double minTurn = NormalizeAngle(angleDiff);
