   ...
   public:
    RKNNQuadNode() : Node("rknn_quad_node") {
        ...
        this->declare_parameter<std::string>(
            "trot_model_path",
            "/home/cat/hitcrt_quad_ws/src/inference/rknn_quad_node/models/"
            "trot.rknn");
        this->declare_parameter<std::string>(
            "creep_model_path",
            "/home/cat/hitcrt_quad_ws/src/inference/rknn_quad_node/models/"
        ...
        this->declare_parameter<std::string>(
            ...
            "/home/cat/hitcrt_quad_ws/src/inference/rknn_quad_node/models/"
            "black_kneel_crawl.rknn");

        std::string trot_model_path =
            this->get_parameter("trot_model_path").as_string();
        std::string creep_model_path =
            this->get_parameter("creep_model_path").as_string();
        std::string upstair_model_path =
            this->get_parameter("upstair_model_path").as_string();
        std::string kneel_crawl_model_path =
            this->get_parameter("kneel_crawl_model_path").as_string();
        ...
        // 2. 创建发布者
        output_pub_ = this->create_publisher<std_msgs::msg::Float32MultiArray>(
            "quad_output", 10);
        inference_time_pub_ = this->create_publisher<std_msgs::msg::Float64>(
            "inference_time", 10);
        ...
        observation_sub_ =
            this->create_subscription<std_msgs::msg::Float32MultiArray>(
                "/quad/obs_data", 10,
                std::bind(&RKNNQuadNode::observation_callback, this,
                          std::placeholders::_1));
        // // 订阅遥控器命令，用于切换模型
        // high_cmd_sub_ = this->create_subscription<quad::msg::HighCommands>(
        //     "/quad/high_command",
        //     10,
        //     std::bind(&RKNNQuadNode::high_command_callback, this,
        //     std::placeholders::_1)
        // );
        // 订阅策略切换命令，用于切换模型
        policy_select_sub_ = this->create_subscription<std_msgs::msg::Int8>(
            "/quad/policy_select", 10,
            std::bind(&RKNNQuadNode::policy_select_callback, this,
                      std::placeholders::_1));

        RCLCPP_INFO(this->get_logger(),
                    "RKNN 节点已启动 (Topic Subscription Mode)");
        RCLCPP_INFO(this->get_logger(), "当前模型: %s (索引: %zu)",
                    model_list_[current_model_index_].c_str(),
                    current_model_index_);
        RCLCPP_INFO(
            this->get_logger(),
            "模型切换规则: RL_MOVE->模型0, RL_LOW->模型1, RL_HIGH->模型2");
    }
   ...
   private:
    ...
    void switch_model(size_t target_index) {
        if (target_index >= model_list_.size()) {
            RCLCPP_ERROR(this->get_logger(), "模型索引超出范围: %zu",
                         target_index);
            return;
        }
        ...
        RCLCPP_INFO(
            this->get_logger(), "模型切换成功！当前模型: %s (索引: %zu)",
    ...
    bool init_rknn_standard(const std::string& model_path) {
        ...
        if (!fp) {
            RCLCPP_ERROR(this->get_logger(), "无法打开模型: %s",
                         model_path.c_str());
            return false;
        }
        fseek(fp, 0, SEEK_END);
        ...
        RCLCPP_INFO(this->get_logger(),
                    "Model Loaded. Input Num: %d, Output Num: %d",
        ...
        // rknn_query(ctx_, RKNN_QUERY_OUTPUT_ATTR, &out_attr,
        // sizeof(out_attr));
    ...
    // --- 订阅回调 & 推理函数 ---
    void observation_callback(
        const std_msgs::msg::Float32MultiArray::SharedPtr msg) {
        // 0. 数据校验
        if (msg->data.size() != INPUT_SIZE) {
            RCLCPP_WARN_THROTTLE(
                this->get_logger(), *this->get_clock(), 1000,
                "接收到的观测数据维度错误! Expected: %d, Got: %zu", INPUT_SIZE,
                msg->data.size());
            return;
        }
        ...
        // 注意：const_cast 是因为 rknn 接口没有声明 const
        // void*，但通常不会修改输入 buffer
        ...
        out_msg.data.assign(output_ptr,
                            output_ptr + OUTPUT_SIZE);  // 只取前12个
