...
class StateMachineNode(Node):
    ...
    def _process_event(self, event_id: EventID):
        ...
        if event_id == EventID.ALLOW_AUTO and self.current_mode_ == ControlMode.AUTO:
            self._publish_task_command(TaskStateID.CMD_ALLOW_LOGISTICS_NAV)
        ...
        if event_id == EventID.PICK_UP:
            ...
            self.last_trigger_time_ = now
            return
        elif event_id == EventID.DISABLE_MANIPULATOR:
            self._disable_manipulator()
            self.last_trigger_time_ = now
            return

    def _disable_manipulator(self):
        msg = UInt8()
        msg.data = 5  # TASK_DISABLE
        self.manip_pub_.publish(msg)
        self.get_logger().warn('Manipulator DISABLE sent (cmd=5)')
    ...
    def _publish_high_command(self):
        ...
        pub_msg.lin_x = self.target_velocity_.linear.x*1.0
        pub_msg.lin_y = min(max(self.target_velocity_.linear.y*0.8,-0.8),0.8)
        pub_msg.ang_yaw = self.target_velocity_.angular.z*1.5
