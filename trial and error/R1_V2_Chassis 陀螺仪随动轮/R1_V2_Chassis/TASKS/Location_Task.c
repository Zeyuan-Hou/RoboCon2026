 ...
 void Follower_Wheel_Location(ST_ROBOT *pstRobot, ST_FOLLOWER_WHEEL *pstFW)
{
    ...
    // 随动轮中心姿态角是指机器人中心至随动轮中心的向量与全局坐标系Y轴的夹角
		...
		Gyro_Data_Test.fpQ_Cur = g_output_info.yaw*10.f;
...
//void Follower_Wheel_Location(ST_ROBOT *pstRobot, ST_FOLLOWER_WHEEL *pstFW)
//{
//    fp32 ALPHA_B;             // B随动轮逆时针旋转时线速度方向与机器人局部坐标系y正方向的夹角
//    fp32 ALPHA_A;             // A随动轮逆时针旋转时线速度方向与机器人局部坐标系y正方向的夹角
//    fp32 fpDeltaA, fpDeltaB;  // 随动轮走过的距离，单位：mm
//    fp32 fpQ, denom;          // 机器人姿态角和联立方程分母
//    fp32 delta_x_local, delta_y_local; // 局部坐标系位移

//    /*******************获取角度*********************/
//    Gyro_Data_Test.fpQ_Cur = 0;//g_output_info.yaw * 10.f;
//    if(fabs(Gyro_Data_Test.fpQ_Cur) > 1800)
//    {
//        Gyro_Data_Test.fpQ_Cur = Gyro_Data_Test.fpQ_Pre;
//    }
//    if(fabs(fabs(Gyro_Data_Test.fpQ_Cur) - 1800) > 15)
//    {
//        if(fabs(Gyro_Data_Test.fpQ_Cur - Gyro_Data_Test.fpQ_Pre) > 100)
//        {
//            Gyro_Data_Test.fpQ_Cur = Gyro_Data_Test.fpQ_Pre;
//        }
//    }
//    if(Gyro_Data_Test.fpQ_Cur - Gyro_Data_Test.fpQ_Pre > 1800)
//        num_circle--;
//    else if(Gyro_Data_Test.fpQ_Cur - Gyro_Data_Test.fpQ_Pre < -1800)
//        num_circle++;
//    
//    fpSumPosQ = Gyro_Data_Test.fpQ_Cur + num_circle * 3600;
//    pstRobot->stPos.fpPosQ = fpSumPosQ - num_circle * 3600;
//    fpQ = ConvertAngle(pstRobot->stPos.fpPosQ * RADIAN_10);
...
//    // 异常检测
//    if(my_intabs(pstFW->siCoderACur - pstFW->siCoderAPre) > 1000 ||
//       my_intabs(pstFW->siCoderBCur - pstFW->siCoderBPre) > 1000)
//    {
//        pstFW->siCoderAPre = pstFW->siCoderACur;
//        pstFW->siCoderBPre = pstFW->siCoderBCur;
//        Gyro_Data_Test.fpQ_Pre = Gyro_Data_Test.fpQ_Cur;
...
//    // 距离和角度计算（与MATLAB正反转定义完全一致）
//    if(pstFW->siCoderACur >= pstFW->siCoderAPre) // A轮正转，编码器变大
//    {
//        fpDeltaA = (fp32)(pstFW->siCoderACur - pstFW->siCoderAPre) * FW_Len_A_Inc;
//        ALPHA_A = ALPHA_A_Inc + PI; // 与MATLAB的+pi补偿完全一致
//    }
//    else // A轮反转，编码器变小
//    {
//        fpDeltaA = (fp32)(pstFW->siCoderACur - pstFW->siCoderAPre) * FW_Len_A_Dec;
//        ALPHA_A = ALPHA_A_Dec + PI; // 与MATLAB的+pi补偿完全一致
//    }

//    if(pstFW->siCoderBCur >= pstFW->siCoderBPre) // B轮正转，编码器变大（对应MATLAB的YBup/XBdown）
//    {
//        fpDeltaB = (fp32)(pstFW->siCoderBCur - pstFW->siCoderBPre) * FW_Len_B_Inc;
//        ALPHA_B = ALPHA_B_Inc; // B轮无pi补偿，与MATLAB一致
//    }
//    else // B轮反转，编码器变小（对应MATLAB的YBdown/XBup）
//    {
//        fpDeltaB = (fp32)(pstFW->siCoderBCur - pstFW->siCoderBPre) * FW_Len_B_Dec;
//        ALPHA_B = ALPHA_B_Dec; // B轮无pi补偿，与MATLAB一致
//    }

//    /**************解联立方程计算局部位移（核心修正，对齐MATLAB模型）****************/
//    denom = sinf(ALPHA_B - ALPHA_A);
//    if(fabs(denom) < 1e-6f) // 避免除以零
//    {
//        pstFW->siCoderAPre = pstFW->siCoderACur;
//        pstFW->siCoderBPre = pstFW->siCoderBCur;
//        Gyro_Data_Test.fpQ_Pre = Gyro_Data_Test.fpQ_Cur;
...
//    // 严格按照MATLAB核心方程解出的局部坐标系位移
//    delta_x_local = (fpDeltaA * cosf(ALPHA_B) - fpDeltaB * cosf(ALPHA_A)) / denom;
//    delta_y_local = (fpDeltaA * sinf(ALPHA_B) - fpDeltaB * sinf(ALPHA_A)) / denom;

//    // 转换到全局坐标系并更新随动轮中心坐标
//    pstFW->stPos.fpPosX += delta_x_local * cosf(fpQ) - delta_y_local * sinf(fpQ);
//    pstFW->stPos.fpPosY += delta_x_local * sinf(fpQ) + delta_y_local * cosf(fpQ);

//    /**************解算机器人中心坐标（与原代码完全一致）****************/
//    pstRobot->stPos.fpPosX = fpStartX + pstFW->stPos.fpPosX 
//                            - (-sinf(FW_rob_Alpha + fpQ) + sinf(FW_rob_Alpha)) * FW_Rob_Len;
//    pstRobot->stPos.fpPosY = fpStartY + pstFW->stPos.fpPosY 
//                            + (-cosf(FW_rob_Alpha + fpQ) + cosf(FW_rob_Alpha)) * FW_Rob_Len;

//    pstRobot->stPos.fpPosX += fpPosXOffset;
//    pstRobot->stPos.fpPosY += fpPosYOffset;
...
//    fp32 ALPHA_B;
//    fp32 ALPHA_A;
//    fp32 fpDeltaA, fpDeltaB;
//    fp32 Convert_Array[2][2];
//    fp32 fpQ;

//    /******************* 强制初始化（仅执行一次）*********************/
//    static bool is_init = false;
//    if (!is_init) {
//        pstFW->siCoderAPre = degreeA;
//        pstFW->siCoderBPre = degreeB;
//        pstFW->stPos.fpPosX = 0.0f;
//        pstFW->stPos.fpPosY = 0.0f;
//        Gyro_Data_Test.fpQ_Pre = 0.0f;
//        num_circle = 0;
//        is_init = true;
//    }
...
//    /******************* 计算位移和角度（与官方标定完全一致）*********************/
//    if (pstFW->siCoderACur >= pstFW->siCoderAPre) {
//        fpDeltaA = (fp32)(pstFW->siCoderACur - pstFW->siCoderAPre) * FW_Len_A_Inc;
//        ALPHA_A = ALPHA_A_Inc;
//    } else {
//        fpDeltaA = (fp32)(pstFW->siCoderACur - pstFW->siCoderAPre) * FW_Len_A_Dec;
...
//    if (pstFW->siCoderBCur >= pstFW->siCoderBPre) {
//        fpDeltaB = (fp32)(pstFW->siCoderBCur - pstFW->siCoderBPre) * FW_Len_B_Inc;
//        ALPHA_B = ALPHA_B_Inc;
//    } else {
//        fpDeltaB = (fp32)(pstFW->siCoderBCur - pstFW->siCoderBPre) * FW_Len_B_Dec;
...
//    /******************* 更新随动轮坐标 *********************/
//    fp32 delta_x = Convert_Array[0][0] * fpDeltaA + Convert_Array[0][1] * fpDeltaB;
//    fp32 delta_y = Convert_Array[1][0] * fpDeltaA + Convert_Array[1][1] * fpDeltaB;

//    // 过滤微小噪声（可选，官方代码中没有，但可以防止漂移）
//    if (fabsf(delta_x) < 0.001f) delta_x = 0.0f;
//    if (fabsf(delta_y) < 0.001f) delta_y = 0.0f;

//    pstFW->stPos.fpPosX += delta_x;
//    pstFW->stPos.fpPosY += delta_y;
