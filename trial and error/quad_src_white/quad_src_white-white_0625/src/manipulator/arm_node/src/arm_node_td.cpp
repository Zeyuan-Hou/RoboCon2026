 ...
 * - /quad/arm/disable            : 失能信号 (std_msgs/Bool: true=强制电机电流归零)
 *
 * 发布话题：
...
enum ArmState : uint8_t {
    ...
    LAY = 5,         ///< 平放状态
    ARM_TRACKING = 6, ///< 轨迹跟踪状态（manager通知arm开始轨迹跟踪）
};
...
enum class TrajectoryPhase {
    ...
    WAIT_FOR_TRACKING,      ///< APPROACH到位后等待manager发送ARM_TRACKING命令
    TRACKING,               ///< 轨迹跟随阶段
    SHAKE                   ///< 腕部正弦往复阶段
};
...
public:
    ...
    ArmNodeTD()
        ...
        // ========== 声明机械臂参数 ==========
        // 控制参数
        this->declare_parameter("control_freq_hz", 500.0);  ///< 控制频率(Hz)
        ...
        this->declare_parameter("td_h", 0.1);              ///< TD滤波因子h，越大滤波越好
        this->declare_parameter("tracking_td_r", 30.0);    ///< TRACKING阶段TD速度因子
        this->declare_parameter("tracking_td_h", 0.1);     ///< TRACKING阶段TD滤波因子

        // 电机0 PID参数 (控制大臂)
        this->declare_parameter("motor0_Kp", 6000.0);
        this->declare_parameter("motor0_Ki", 0.5);
        this->declare_parameter("motor0_Kd", 0.2);

        // 电机1 PID参数 (控制小臂)
        this->declare_parameter("motor1_Kp", 3000.0);
        this->declare_parameter("motor1_Ki", 0.2);
        this->declare_parameter("motor1_Kd", 0.2);

        // 重力补偿系数
        this->declare_parameter("grav_comp_cur", 1000.0);
        ...
        this->declare_parameter("lift_motor1_pos", -0.8);     ///< LIFT状态小臂目标位置
        this->declare_parameter("place_low_motor0_pos", 1.0); ///< PLACE_LOW状态大臂目标位置
        this->declare_parameter("place_low_motor1_pos", 1.3);///< PLACE_LOW状态小臂目标位置
        this->declare_parameter("place_high_motor0_pos", 3.0);///< PLACE_HIGH状态大臂目标位置
        this->declare_parameter("place_high_motor1_pos", 0.5);///< PLACE_HIGH状态小臂目标位置
        this->declare_parameter("lay_motor0_pos", 0.0);       ///< LAY状态大臂目标位置
        this->declare_parameter("lay_motor1_pos", 0.0);       ///< LAY状态小臂目标位置

        // 偏置
        this->declare_parameter("motor0_zero_offset", -4.5);   ///< 电机0零位偏置
        ...
        // APPROACH外部就绪标志参数
        this->declare_parameter("approach_ready_timeout_s", 10.0);  ///< 等待外部就绪超时时间(s)
        this->declare_parameter("approach_ready_topic", std::string("/quad/logistics_pickup_reached"));  ///< 外部就绪标志话题
        this->declare_parameter("tracking_wait_timeout_s", 10.0);  ///< 等待ARM_TRACKING命令的超时时间(s)
        ...
        // 订阅外部就绪标志（机器人落位后发布）
        std::string approach_ready_topic = this->get_parameter("approach_ready_topic").as_string();
        approach_ready_sub_ = this->create_subscription<std_msgs::msg::Bool>(
            approach_ready_topic, 10,
            std::bind(&ArmNodeTD::approach_ready_callback, this, std::placeholders::_1));
        RCLCPP_INFO(this->get_logger(), "Subscribed to approach_ready topic: %s", approach_ready_topic.c_str());

        // 订阅 manager 的失能信号（电流归零）
        arm_disable_sub_ = this->create_subscription<std_msgs::msg::Bool>(
            "/quad/arm/disable", 10,
            std::bind(&ArmNodeTD::arm_disable_callback, this, std::placeholders::_1));
        RCLCPP_INFO(this->get_logger(), "Subscribed to arm disable topic: /quad/arm/disable");
...
private:
    ...
    void switch_arm_state(ArmState new_state) {
        ...
        if (new_state <= NONE || new_state > ARM_TRACKING) {
        ...
        if (new_state == GRAB && use_trajectory_for_grab_) {
            ...
            robot_ready_received_ = false;
            pending_tracking_ = false;
            approach_ready_start_time_ = this->now();
            // 重置轨迹完成信号
            std_msgs::msg::Bool complete_msg;
            complete_msg.data = false;
            trajectory_complete_pub_->publish(complete_msg);
            // 显式发布arrived=false，确保manager收到fresh信号
        ...
        const char* state_names[] = {"NONE", "GRAB", "LIFT", "PLACE_LOW", "PLACE_HIGH", "LAY", "ARM_TRACKING"};
    ...
    void arm_state_cmd_callback(const std_msgs::msg::UInt8::SharedPtr msg) {
        if (msg->data == ARM_TRACKING) {
            // manager通知arm开始轨迹跟踪
            if (trajectory_phase_ == TrajectoryPhase::WAIT_FOR_TRACKING) {
                ...
                pending_tracking_ = false;
                RCLCPP_INFO(this->get_logger(),
                    "ARM_TRACKING received, starting trajectory tracking");
            } else {
                // 命令到达时还未进入WAIT_FOR_TRACKING，设置待处理标志
                // 等进入WAIT_FOR_TRACKING后自动开始追踪
                pending_tracking_ = true;
                RCLCPP_INFO(this->get_logger(),
                    "ARM_TRACKING received in phase %d, pending until WAIT_FOR_TRACKING",
                    static_cast<int>(trajectory_phase_));
            }
        } else {
    ...
    void approach_ready_callback(const std_msgs::msg::Bool::SharedPtr msg) {
        robot_ready_received_ = msg->data;
        if (robot_ready_received_) {
            RCLCPP_INFO(this->get_logger(), "Approach ready flag received: true");
        }
    }

    void arm_disable_callback(const std_msgs::msg::Bool::SharedPtr msg) {
        if (!msg->data) {
            // false 不改变状态，恢复由 /quad/arm/state_cmd 重新触发。
            arm_disable_active_ = false;
            return;
        }
        // true -> 强制进入 NONE 状态。每拍 motor_cmd = {0.0, 0.0}，电流归零。
        // 仅在上升沿（首次进入失能）打日志，避免每拍刷屏。
        if (!arm_disable_active_) {
            current_state_ = NONE;
            // 清理可能正在进行的轨迹状态，避免恢复时被陈旧相位干扰
            trajectory_phase_ = TrajectoryPhase::NONE;
            trajectory_finished_ = false;
            pending_tracking_ = false;
            RCLCPP_WARN(this->get_logger(),
                        "Arm DISABLE engaged: motor current forced to 0 (state->NONE)");
            arm_disable_active_ = true;
        } else {
            // 维持失能：每拍只需保持 NONE，无需重复日志/清理。
            current_state_ = NONE;
        }
    }
            ...
            if (trajectory_phase_ == TrajectoryPhase::APPROACH) {
                if (!trajectory_.empty()) {
                    ...
                    if (td_tracker_[0].hasArrived(0.01, 0.05) && td_tracker_[1].hasArrived(0.01, 0.05)) {
                        trajectory_phase_ = TrajectoryPhase::WAIT_FOR_TRACKING;
                        tracking_wait_start_time_ = this->now();
                        RCLCPP_INFO(this->get_logger(),
                            "Approach completed, waiting for ARM_TRACKING command (timeout=%.1fs)",
                            this->get_parameter("tracking_wait_timeout_s").as_double());
                    }
                } else {
                    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                        "Trajectory is empty! Check trajectory file.");
                }
            } else if (trajectory_phase_ == TrajectoryPhase::WAIT_FOR_TRACKING) {
                double wait_elapsed = (this->now() - tracking_wait_start_time_).seconds();
                double timeout_s = this->get_parameter("tracking_wait_timeout_s").as_double();

                // 保持当前位置不动，等待manager发送ARM_TRACKING命令
                arm_goal_pos[0] = trajectory_[0].pos[0];
                arm_goal_pos[1] = trajectory_[0].pos[1];
                td_tracker_[0].setTarget(arm_goal_pos[0]);
                td_tracker_[1].setTarget(arm_goal_pos[1]);

                if (pending_tracking_) {
                    // ARM_TRACKING命令已预到达，立即开始追踪
                    trajectory_phase_ = TrajectoryPhase::TRACKING;
                    trajectory_start_time_ = this->now();
                    td_tracker_[0].setParameters(tracking_td_r_, tracking_td_h_);
                    td_tracker_[1].setParameters(tracking_td_r_, tracking_td_h_);
                    ...
                    pending_tracking_ = false;
                    RCLCPP_INFO(this->get_logger(),
                        "Pending ARM_TRACKING applied, starting trajectory tracking");
                } else if (wait_elapsed >= timeout_s) {
                    // 超时回退：直接进入TRACKING
                    trajectory_phase_ = TrajectoryPhase::TRACKING;
                    trajectory_start_time_ = this->now();
                    td_tracker_[0].setParameters(tracking_td_r_, tracking_td_h_);
                    ...
                    td_tracker_[1].reset(current_pos[1]);
                    RCLCPP_WARN(this->get_logger(),
                        "ARM_TRACKING timeout after %.1fs, proceeding to tracking anyway", wait_elapsed);
                }
                ...
                if (shake_elapsed < shake_duration_) {
                    ...
                    trajectory_complete_pub_->publish(complete_msg);
                    RCLCPP_INFO(this->get_logger(), "SHAKE completed, trajectory_complete published");
                }
            }
        } else {
    ...
    // APPROACH外部就绪标志
    bool robot_ready_received_ = false;
    rclcpp::Time approach_ready_start_time_;
    rclcpp::Time tracking_wait_start_time_;  ///< 等待ARM_TRACKING命令的起始时间
    bool pending_tracking_ = false;           ///< ARM_TRACKING命令预到达标志（用于处理命令先于WAIT_FOR_TRACKING到达的情况）
    bool arm_disable_active_ = false;          ///< 当前是否处于失能状态（用于上升沿单次日志）
    ...
    rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr arm_disable_sub_;   ///< 失能信号订阅：true=强制 current_state_=NONE
    ...
    rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr trajectory_complete_pub_;  ///< 轨迹完成信号
