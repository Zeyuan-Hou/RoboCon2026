	...
	if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
	{
		...
		if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &RxHeader, RxData) == HAL_OK)
		{
			...
			if (hfdcan->Instance == FDCAN1)
			{
				...
				case 0x202:
					stretch_2006.EncoderNum = GetEncoderNumber(&stretch_2006, RxData);
					stretch_2006.encoder_speed = GetSpeed(&RxHeader, RxData);
			...
			else if (hfdcan->Instance == FDCAN2)
			{
				...
				{
				case 0x201:
					DJI_3508.EncoderNum = GetEncoderNumber(&DJI_3508, RxData);
					DJI_3508.encoder_speed = GetSpeed(&RxHeader, RxData);
					Abs_Encoder_Process(&DJI_3508.motor_encoder, DJI_3508.EncoderNum);
					DJI_3508.angle = DJI_3508.motor_encoder.siSumValue / (float)8192 * 360.f / (float)M3508_uiGearRatio - DJI_3508.Start_Pos;
					if (!DJI_3508.getStartPos)
					{
						DJI_3508.Start_Pos = DJI_3508.angle;
						DJI_3508.getStartPos = 1;
					}
					DJI_3508.anglev = DJI_3508.encoder_speed / (float)M3508_uiGearRatio;
