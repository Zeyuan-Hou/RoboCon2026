...
class LidarNavControl(Node):
    ...
    def tf_callback(self, msg: TFMessage):
        ...
        for transform in msg.transforms:
            if transform.header.frame_id == self.target_frame and transform.child_frame_id == self.child_frame:
                self.x = transform.transform.translation.y
                self.y = transform.transform.translation.x
    ...
    def control_loop(self):
        ...
        if self.enable_control:
            cmd.linear.x = max(-self.max_vx, min(self.kp_xy * err_x, self.max_vx))
            cmd.linear.y = max(-self.max_vy, min(self.kp_xy * -err_y, self.max_vy))
