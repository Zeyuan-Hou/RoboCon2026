#ifndef __AIR_BOARD_H__
#define __AIR_BOARD_H__

#include "stm32f4xx.h"
#include "string.h"

#define CHASSIS_U CHANNEL_1
#define CHASSIS_LD CHANNEL_2
#define CHASSIS_RD CHANNEL_3
...
//气缸有关宏定义 (1-8)
#define OPEN_VALVE(channel)   if(1){( g_uiAirValve |= (0x01 << (channel-1)) ) ; SendAirMsgByCan2_Plus(&g_uiAirValve);}
#define CLOSE_VALVE(channel)  if(1){( g_uiAirValve &= ( ~(0x01 << (channel-1) ) )) ;	SendAirMsgByCan2_Plus(&g_uiAirValve);}
...
#define SET_VALVE_VALUE(value)    (g_uiAirValve = value);SendAirMsgByCan1(&g_uiAirValve)
#define CHANGE_VALVE(channel)        (g_uiAirValve ^= (((uint32_t)(0x01<<(channel-1)))));SendAirMsgByCan1(&g_uiAirValve)
...
extern uint32_t g_uiAirValve_Plus;
...
extern void SendServoMsgByCan2(int16_t value1, int16_t value2, int16_t value3, int16_t value4);
