...
RLControlNode::RLControlNode() : Node("rl_interface_node") {
    ...
    state_sub_ = this->create_subscription<quad::msg::State>(
        "/quad/test_estm_state", 10, std::bind(&RLControlNode::state_callback, this, std::placeholders::_1));
    encoder_sub_ = this->create_subscription<quad::msg::Encoder>(
        "/quad/encoder", 10, std::bind(&RLControlNode::encoder_callback, this, std::placeholders::_1));

    high_cmd_sub_ = this->create_subscription<quad::msg::HighCommands>(
        "/quad/high_command", 10, std::bind(&RLControlNode::high_command_callback, this, std::placeholders::_1));

    action_sub_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
        "/quad/quad_output", 10, std::bind(&RLControlNode::rl_action_callback, this, std::placeholders::_1));

    // 发布者
    obs_pub_ = this->create_publisher<std_msgs::msg::Float32MultiArray>("/quad/obs_data", 10);
    joint_cmd_pub_ = this->create_publisher<quad::msg::JointCmd>("/quad/joint_cmd", 10);
    policy_select_pub_ = this->create_publisher<std_msgs::msg::Int8>("/quad/policy_select", 10);

    // 定时器 (50Hz)
    timer_ = this->create_wall_timer(
        std::chrono::duration<double>(0.02), std::bind(&RLControlNode::update_rl_obs, this));
    ...
    this->declare_parameter("default_angles_kneel_crawl", DEFAULT_ANGLES_KNEEL_CRAWL);
    default_angles_trot_ = this->get_parameter("default_angles_trot").as_double_array();
    default_angles_creep_ = this->get_parameter("default_angles_creep").as_double_array();
    default_angles_upstair_ = this->get_parameter("default_angles_upstair").as_double_array();
    default_angles_kneel_crawl_ = this->get_parameter("default_angles_kneel_crawl").as_double_array();
    
    default_dof_angles_ = default_angles_trot_; // 默认为trot

    obs_queue_.assign(obs_history_length_, std::array<double, 45>{}); // 初始化观测队列长度,防止
}
...
void RLControlNode::high_command_callback(const quad::msg::HighCommands::SharedPtr msg) {
    ...
    if (msg->policy_switch != current_policy_) {
        ...
        if (current_policy_ == TROT) {
            ...
            // RCLCPP_INFO(this->get_logger(), "Switched to CREEP default angles");
        } else if(current_policy_ == CREEP) {
            default_dof_angles_ = default_angles_creep_;
            // RCLCPP_INFO(this->get_logger(), "Switched to TROT default angles");
...
void RLControlNode::rl_action_callback(const std_msgs::msg::Float32MultiArray::SharedPtr msg) {
    ...
    // 获取当前Action并限幅
    for (int i = 0; i < 12; ++i) {
        current_action[i] = std::clamp((double)msg->data[i], 
...
{
    std::ofstream ofs(obs_filename_, std::ios::out | std::ios::trunc); // 覆盖写入文件

    if (!ofs.is_open()) {
        std::cerr << "Error: Could not open file " << obs_filename_ << " for writing." << std::endl;
        return;
    }
    ...
    ofs.close();
    // std::cout << "\n✅ Observation data (Total " << obs_queue_.size() * 45 << " values) saved to " << obs_filename_ << std::endl;
}

Vec3 RLControlNode::quatRotateInverse(const Vec3& v, float x, float y, float z, float w) {
    Vec3 r{-x, -y, -z};
    Vec3 t;
...
void RLControlNode::update_rl_obs() {
    ...
    Vec3 g_body = quatRotateInverse(g_world, sensor_.imu.x, sensor_.imu.y, sensor_.imu.z, sensor_.imu.w);
    obs[6] = g_body.x;
    obs[7] = g_body.y;
    obs[8] = g_body.z;
    ...
    // 9-20: 相对默认角度的关节位置
    for (int i = 0; i < 12; i++) {
        obs[9 + i] = (sensor_.encoder.qpos[i] - default_dof_angles_[i]) * normalization_.obs_scales.dof_pos;
    }
    // 21-32: 关节速度
    for (int i = 0; i < 12; i++) {
        obs[21 + i] = sensor_.encoder.qvel[i] * normalization_.obs_scales.dof_vel;
    }
    // 33-44: 上一步 Action
