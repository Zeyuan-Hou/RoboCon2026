...
void BlockArm_Logic(void)
{
    ...
    case BLOCKARM_TASK_FETCH_BLOCK://取台阶上的方块时执行的吸取与收回动作
            ...
            case 1:
                ...
                {
                    Traj_Line_Init(&Traj_Arm, Arm_BackSolving.x, Arm_BackSolving.y,200.f, 380.f, 0.f, 0.f, 2000.f, 10000.f);
				    MovementFlag[0]=1;
                }
                break;
    ...
    case BLOCKARM_TASK_RETRACT_ARM://收回方块到存储位置
        ...
        {	
            ...
            {
				Traj_Angle_Init(&Traj_BlockArm_Joint3, MotorInput.BlockArm_Joint3_3508, -90.f, 360.f, 0.5f);
				Traj_Line_Init(&Traj_Arm, Arm_BackSolving.x, Arm_BackSolving.y,300.f, 380.f, 0.f, 0.f, 800.f, 5000.f);
				MovementFlag[1]=1;
            }
            else if(MovementFlag[1]==1&&MovementFlag[2]==0&&Traj_Arm.State==TRAJ_DONE&&Traj_BlockArm_Joint3.State==TRAJ_DONE)
            {
                Traj_Angle_Init(&Traj_Gimbal,MotorInput.BlockArm_Gimbal_J60, 180.f, 360.f, 0.5f);
				MovementFlag[2]=1;
            }
...
//        }  
        ...
        else if (BlockArm_Task_Change.BlockStore[2]==0&&BlockArm_Task_Change.BlockStore_State==3)
        {
			...
			if(MovementFlag[0]==0&&Traj_Arm.State==TRAJ_DONE)
			{
				...
				Traj_Angle_Init(&Traj_BlockArm_Joint3, MotorInput.BlockArm_Joint3_3508, 0.f, 60.f, 3.f);
				MovementFlag[0]=1;
			}
			else if(MovementFlag[0]==1&&MovementFlag[1]==0&&Traj_Gimbal.State==TRAJ_DONE&&Traj_Arm.State==TRAJ_DONE&&Traj_BlockArm_Joint3.State==TRAJ_DONE)
			{				
    ...
    case BLOCKARM_TASK_PICKUP_BLOCK://三区拿起存储的方块
		...
		else if (BlockArm_Task_Change.BlockStore[0]==1)
        {
			if(MovementFlag[0]==0&&Traj_Arm.State==TRAJ_DONE)
            {
                Traj_Angle_Init(&Traj_Gimbal,MotorInput.BlockArm_Gimbal_J60, 180.f, 360.f, 0.8f);
			...
			else if(MovementFlag[1]==1&&MovementFlag[2]==0&&Traj_Gimbal.State==TRAJ_DONE&&Traj_Arm.State==TRAJ_DONE&&Traj_BlockArm_Joint3.State==TRAJ_DONE)
            {
                ...
                vTaskDelay(pdMS_TO_TICKS(500));
                Traj_Line_Init(&Traj_Arm, Arm_BackSolving.x, Arm_BackSolving.y,250.f, 400.f, 0.f, 0.f, 1000.f, 10000.f);
				Traj_Angle_Init(&Traj_Gimbal,MotorInput.BlockArm_Gimbal_J60, 0.f, 240.f, 0.8f);
				MovementFlag[2]=1;
            }
    ...
    case BLOCKARM_TASK_PUT_BLOCK:
		...
		if(MovementFlag[0]==0&&Traj_Arm.State==TRAJ_DONE)
        {
			if(BlockArm_Task_Change.Block_Height!=0)
			{
				Traj_Line_Init(&Traj_Arm, Arm_BackSolving.x, Arm_BackSolving.y,450.f, 250.f, 0.f, 0.f, 500.f, 2000.f);
			}
			MovementFlag[0]=1;
        }
...
{
    if (p_vision_data.x2==0&&p_vision_data.y2==0&&p_vision_data.yaw2==0&&p_vision_data.yaw3==0&&p_vision_data.distance==0)
    {
...
{   
    ...
    switch (PoleArm_Task_State)
    {
        ...
        case POLEARM_TASK_MOVE_TO_FETCH_POSITION:
            ...
            // 如果已经到达取杆位置，切换状态
            if (PoleArm_State_Judge(3.f,3.f,3.f)&&PoleArm_Task_Timer.POLEARM_TASK_MOVE_TO_FETCH_POSITION>=500)
            {
        ...
        case POLEARM_TASK_MOVE_TO_PLACE_POSITION:
            ...
            // 如果已经到达放杆位置，切换状态
            if (PoleArm_State_Judge(6.f,5.f,5.f)&&PoleArm_Task_Timer.POLEARM_TASK_MOVE_TO_PLACE_POSITION>=3000) 
			{
...
void USART1_Receive_Data(uint8_t Communicate_KEY)
{
    ...
    //三区任务
    else if (Communicate_KEY==4&&BlockArm_Task_Change.Area_State==3&&(BlockArm_Task_State==BLOCKARM_TASK_INIT||BlockArm_Task_State==BLOCKARM_TASK_RETRACT_ARM))
    {
