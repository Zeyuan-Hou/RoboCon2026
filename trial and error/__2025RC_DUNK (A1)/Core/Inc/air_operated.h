...
//气缸有关宏定义 (1-8)
#define OPEN_VALVE(channel)   if(1){( g_uiAirValve |= (0x01 << (channel-1)) ) ;}
#define CLOSE_VALVE(channel)  if(1){( g_uiAirValve &= ( ~(0x01 << (channel-1) ) ));}
#define IS_VALVE_OPEN(channel)    (g_uiAirValve &(0x01<<(channel-1)))
#define IS_VALVE_CLOSE(channel)   (!(g_uiAirValve &(0x01<<(channel-1))))
#define GET_VALVE_VALUE()             (g_uiAirValve)
#define SET_VALVE_VALUE(value)    (g_uiAirValve = value);SendAirMsgByCan1(&g_uiAirValve)
#define CHANGE_VALVE(channel)        (g_uiAirValve ^= (((uint32_t)(0x01<<(channel-1)))));
