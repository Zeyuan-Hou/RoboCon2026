...
{
  MotorData_t go1_receive_data;	
	memcpy(&go1_receive_data.motor_recv_data,receive_data,sizeof(RIS_MotorData_t));
	extract_data(&go1_receive_data);
	
	switch (receive_data->mode.id)
    {
    case 0x01:
			data_1 = go1_receive_data;
			if(cmd_1.flag_init == 0)//记录此时初始角度
			{
				cmd_1.init_rad = data_1.Pos;
				cmd_1.flag_init = 1;
			}
			cmd_1.feedback_test = (data_1.Pos -cmd_1.init_rad)/6.33f;
			monitor.rate_cnt.lift_201++;
		break;
			
    case 0x02:
			data_2 = go1_receive_data;
			if(cmd_2.flag_init == 0)//记录此时初始角度
			{
				cmd_2.init_rad = data_2.Pos;
				cmd_2.flag_init = 1;
			}
			cmd_2.feedback_test = (data_2.Pos -cmd_2.init_rad)/6.33f;
			monitor.rate_cnt.lift_202++;
		break;
			
    case 0x03:
			data_3 = go1_receive_data;
			if(cmd_3.flag_init == 0)//记录此时初始角度
			{
				cmd_3.init_rad = data_3.Pos;
				cmd_3.flag_init = 1;
			}
			cmd_3.feedback_test = (data_3.Pos -cmd_3.init_rad)/6.33f;
			monitor.rate_cnt.lift_203++;
		break;
			
    case 0x04:
			data_4 = go1_receive_data;
			if(cmd_4.flag_init == 0)//记录此时初始角度
			{
				cmd_4.init_rad = data_4.Pos;
				cmd_4.flag_init = 1;
			}
			cmd_4.feedback_test = (data_4.Pos -cmd_4.init_rad)/6.33f;
			monitor.rate_cnt.lift_204++;
		break;
...
{
		if(ctrl_data->mode == 0)// 清零
	{
		td->aim = ctrl_data->init_rad;
		td->x = 0;
		td->x1 = ctrl_data->init_rad ;	
		td->x2 = 0;	
		ctrl_data->mode = 1;
	}
	else
	{
		td->aim = ctrl_data->Target_Delta_Deg* 6.33f + ctrl_data->init_rad;
		CalTD(td);
		ctrl_data->target_td = (td->x1 - ctrl_data->init_rad)/6.33f;
	}
	
	ctrl_data->Pos = td->x1;
	ctrl_data->W = td->x2;
	
	modify_data(ctrl_data);
	HAL_UART_Transmit_DMA(&huart1,(uint8_t *)&ctrl_data->motor_send_data,sizeof(ctrl_data->motor_send_data));
}
