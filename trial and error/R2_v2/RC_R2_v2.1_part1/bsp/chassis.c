...
void SpeedDistribute_Four_SteeringWheel(ST_VECTOR global_val, ST_Nav *p_nav)
{
    float fpQ;
    if (p_nav->nav_state == RC_LOCAL)
    { // 以车身坐标系操控
        fpQ = 0.f;
    }else{
        // 以全场坐标系操控
        fpQ = ConvertAngle(stRobot.stPos.fpPosQ);
    }
    Convert_Velt_Global2Local(global_val, &expect_robot_local_Velt, fpQ);
...
void Drive_Chassis()
{
    ...
    friction_feedforward.down = Path_Point.extra_ff[Path_Point.point_inx] * steer_velt.down;
    friction_feedforward.leftup = Path_Point.extra_ff[Path_Point.point_inx] * steer_velt.leftup;
    friction_feedforward.rightup = Path_Point.extra_ff[Path_Point.point_inx] * steer_velt.rightup;
