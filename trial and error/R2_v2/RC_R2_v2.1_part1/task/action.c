...
void Task_Across_Forest()
{
    switch (ace.across){
        case from_One_to_2_1:
            ...
            if ((ace.flag_ == 1 && nav.nav_state == NAV_LOCK) || (ace.flag__ == 1 && nav.nav_state == NAV_LOCK))
            {
                ace.flag__ = 0;
                // 如果上台阶，在收后脚时启动底盘闭环控制
                // 进一步地，下台阶也可在小脚坐下时启动底盘闭环控制，这一版还没开发！！
                if (2 * ace.across + 1 == 2 || 2 * ace.across + 1 == 3)
                {
                    nav.auto_path.number_point = decision_tree[ace.path_inx][2 * ace.across + 2];
                    Path_Point.flag_point_to_point = 1;
                    ace.flag__ = 1;
                }
