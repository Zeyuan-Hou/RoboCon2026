...
/**J60_Func**/
u32 FloatToUint(const float x, const float x_min, const float x_max, const uint8_t bits){
...
void CAN_Send_DeepMsg(MotorCMD *motorCMD,hfdcan_t* hfdcan)
{
	FloatsToUints(motorCMD,CAN1_TxBuf);
	uint32_t ID =0x80+motorCMD->motor_id_;//Éú³ÉJ60·¢ËÍ CAN ID
	CAN_SendStdData(hfdcan,ID,CAN1_TxBuf,8);
}

void Deep_Init(DEEP_MOTOR *motor,MotorCMD *motorCMD,hfdcan_t* hfdcan){
	SetMotionCMD(motorCMD,motor->motor_id_,2,0,0,0,0,0);
	FloatsToUints(motorCMD,CAN1_TxBuf);
	uint32_t ID =FormCanId((uint8_t)2,motor->motor_id_);
	CAN_SendStdData(hfdcan,ID,CAN1_TxBuf,0);
}
