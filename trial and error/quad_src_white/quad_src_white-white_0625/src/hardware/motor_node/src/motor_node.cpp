...
void MotorNode::updateMotorFeedback(void) {
    ...
    for (int i = 0; i < 12; i++) {
        ...
        // 错误检测与打印
        if(motor_data.error > 0.1) error_detected_in_this_frame = true; 
...
double MotorNode::limitJointPosition(int joint_idx){
    /* 参数设置 */
    float base_error_limit = 1.0;    // 最大error, 计算: tau_limit / 40
...
void MotorNode::param_init(void) {
    ...
    // 【新增】日志文件初始化 (指定绝对路径)
    // ==========================================
    std::string log_path = "/home/cat/hitcrt_quad2026_ws/src/hardware/motor_node/log/motor_log.csv";
