...
- 实现四种任务流程：
  ...
  - **DISABLE** (5): 失能任务，立即停止机械臂电机电流并关闭气泵。可作为安全中断使用，会抢占任何正在运行的任务，并持续保持失能状态直到收到下一条指令。
...
# DISABLE - 失能（紧急停止机械臂电流+关闭气泵，可抢占任何任务）
ros2 topic pub /manipulator/cmd std_msgs/msg/UInt8 "data: 5" --once
```
...
| 话题名 | 类型 | 说明 |
|--------|------|------|
| `/manipulator/cmd` | `std_msgs/UInt8` | 顶层控制命令<br>1=GRAB, 2=PLACE_LOW, 3=PLACE_HIGH, 5=DISABLE |
...
| `/quad/arm/disable` | `std_msgs/Bool` | 机械臂失能信号 (DISABLE 任务下持续发布 true) |
...
2. 常规任务执行期间（RUNNING状态）会忽略新的命令；但 `TASK_DISABLE=5` 可抢占任何运行中的任务，任何新指令也可抢占进行中的 DISABLE
