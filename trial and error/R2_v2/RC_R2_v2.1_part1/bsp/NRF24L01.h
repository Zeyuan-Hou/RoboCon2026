...
#define ACK_PLOAD_WIDTH 32 // 指定的用户数据宽度
//==========================================================================================================
// NRF24L01寄存器操作命令
...
#define W_ACK_PAYLOAD 0xA8      //+ppp	//RX模式下,将PIPEppp(ppp的范围为000~101)中的AcK包与载荷一起写入,最多可以有3个挂起的ACK包.
#define W_TX_PAYLOAD_NOACK 0xB0 // 发送数据,但禁止自动应答,发射模式用
...
#define RF_CH 0x05      // RF通道,bit6:0,工作通道频率;   //1
#define RF_SETUP 0x06   // RF寄存器;bit3:传输速率(0:1Mbps,1:2Mbps);bit2:1,发射功率;bit0:低噪声放大器增益 //2
...
//==========补充参数
#define DYNPD 0x1C   // nRF24L01 动态有效荷载（数据包长度变化）
#define FEATURE 0x1D // nRF24L01 其他特性设置（ASK/普通）
//==========模式选择
...
uint8_t NRF24L01_Tx_Ack(ACK_PAYLOAD *ack_pay);                            // 处理NRF24L01成功发送并且接收到的应答ACK信号数据包
uint8_t NRF_TX_Packet(uint8_t *txbuf, uint16_t len);                      //(ACK模式) 启动NRF24L01发送一次动态长度数据
void NRF24L01_Rx_AckPayload(ACK_PAYLOAD ack_pay);                         // 发送带数据的 ACK 响应
