...
void LidarNavControl::play_path_ready_audio() {
    if (ocr_audio_played_) {
        return;
    }
    const int audio_id = ocr_result_locked_ ? ocr_result_ : 0;
    const std::string audio_path =
        "/home/cat/hitcrt_quad2026_ws/src/commands/lidar_nav_demo_cpp/records/"
    ...
    ocr_audio_played_ = true;
    RCLCPP_INFO(this->get_logger(),
                "Playing OCR audio: %s (ocr_locked=%s)",
...
void LidarNavControl::generate_path(const std::vector<int>& slot_classes) {
    ...
    const size_t num_dropoff = dropoff_flat_.size() / 3;
    pickup_dock_event_sent_ = false;
    pickup_dock_event_wp_idx_ = 0;
