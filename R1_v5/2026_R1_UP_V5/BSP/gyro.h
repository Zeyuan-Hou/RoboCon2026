...
extern uint8_t g_uart_rx_buf[512];	/*rx DMA buffer of uart2*///串口接收缓冲区
extern uint16_t g_uart_rx_cnt; /*reciede data length of uart2*///表示串口接收缓冲区中接收的数据长度

extern uint8_t g_decode_data[512];	/*buffer for decoding*///数据解码缓冲区
extern uint16_t g_decode_data_pos;	/*bytes left in decode buffer*///解码缓冲区中的当前位置
/*------------------------------------------------Functions declare--------------------------------------------*/
...
extern GYRO_DATA gyro_data,gyro_data_filtered;//欧拉角
