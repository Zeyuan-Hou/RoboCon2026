...
enum class LogisticsEventType : int {
    ...
    PICKUP_DOCK_ENTERED = 4,
    PRE_SCAN_SPIN_DONE = 5
};
...
private:
    ...
    void record_lidar_tf_sample(const rclcpp::Time& now);
    double current_lidar_tf_rate_hz(const rclcpp::Time& now);
    bool lidar_tf_rate_guard_allows_control(
        const rclcpp::Time& now, double& rate_hz, double& age_s, std::string& reason);
    bool lidar_pose_guard_allows_control(std::string& reason);
    void reset_lidar_guard_control_state();
    void publish_lidar_guard_stop();
    ...
    double cross_band_pass_through_dist_arrival_tolerance_;
    ...
    bool lidar_tf_rate_guard_enable_{true};
    double lidar_tf_min_rate_hz_{8.0};
    double lidar_tf_rate_window_s_{1.0};
    double lidar_tf_timeout_s_{0.30};
    std::deque<rclcpp::Time> lidar_tf_sample_times_;
    rclcpp::Time last_lidar_tf_time_;
    bool last_lidar_tf_time_valid_{false};
    bool lidar_tf_rate_guard_active_{false};
    bool lidar_tf_guard_stop_sent_{false};
    bool lidar_pose_guard_enable_{true};
    double lidar_pose_min_x_{-10.0};
    double lidar_pose_max_x_{10.0};
    double lidar_pose_min_y_{-10.0};
    double lidar_pose_max_y_{15.0};
    bool lidar_pose_guard_active_{false};
    ...
    bool pickup_dock_event_sent_{false};
    size_t pickup_dock_event_wp_idx_{0};
    ...
    bool pre_scan_spin_event_sent_{false};
    bool pre_scan_spin_done_time_valid_{false};
    rclcpp::Time pre_scan_spin_done_t0_;
    double pre_scan_ocr_audio_timeout_sec_{5.0};
