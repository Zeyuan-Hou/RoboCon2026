...
void TIM3_IRQHandler(void)
{
  ...
  if (__HAL_TIM_GET_FLAG(&htim3, TIM_FLAG_UPDATE) != RESET)
  {
    ...
    system_monitor.tim++;
    if (system_monitor.tim >= 1000)
    {
      system_monitor.fpsMonitor = system_monitor.cntMonitor;
      memset(&system_monitor.cntMonitor, 0, sizeof(system_monitor.cntMonitor));
      system_monitor.tim = 0;
    }
  }
...
void USART3_IRQHandler(void)
{
  ...
  if (__HAL_UART_GET_FLAG(&huart3, UART_FLAG_IDLE))
  {
    ...
    // huart3.Instance->DR; // clear ore flg
    // huart3.Instance->SR;
    ...
    system_monitor.cntMonitor.gyro++;
  }
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  {
    receive_motor_feedback(usartBufForA1);
    if (a1_init_flag == 0 && Motor_A1.RealPos != 0)
    {
      a1_init_pos = Motor_A1.RealPos;
      a1_init_flag = 1;
    }
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
  ...
  if (huart->Instance == UART4)
  {
	...
	if (huart->Instance == USART3)
	{
			...
			memset(g_uart_rx_buf, 0, 512);
			memset(g_decode_data, 0, sizeof(g_decode_data));

			HAL_UART_Receive_DMA(&huart3, g_uart_rx_buf, 512);
