...
void USART1_IRQHandler(void)
{
  ...
  {            
      __HAL_UART_CLEAR_IDLEFLAG(&huart1);
			...
			received_length = (5 + UART1_RX_DATA_LEN) - remaining;

			uint8_t *current_buf = buffer_flag? uart1_buf1:uart1_buf2;
			if (received_length == (5 + UART1_RX_DATA_LEN))
			{
				if(current_buf[0]==0x55&&current_buf[1]==0x22&&current_buf[2]==UART1_RX_DATA_LEN
															&&current_buf[3+UART1_RX_DATA_LEN]==0x01&&current_buf[4+UART1_RX_DATA_LEN]==0xAA)
				{
					memcpy(uart1_efr.num,&current_buf[3],UART1_RX_DATA_LEN);
					system_monitor.uart1_cnt++;
				}
			}
			buffer_flag = !buffer_flag;
			HAL_UART_Receive_DMA(&huart1, buffer_flag? uart1_buf1:uart1_buf2,5+UART1_RX_DATA_LEN);
...
void UART5_IRQHandler(void)
{
  ...
  {            
      ...
      HAL_UART_DMAStop(&huart5);
			HAL_UART_Receive_DMA(&huart5, uart5_test, 2+4*4);
...
void USART6_IRQHandler(void)
{
  ...
  {            
      ...
      HAL_UART_DMAStop(&huart6);
			HAL_UART_Receive_DMA(&huart6, uart6_test, 25);
