...
void LidarNavControl::record_lidar_tf_sample(const rclcpp::Time& now) {
    last_lidar_tf_time_ = now;
    last_lidar_tf_time_valid_ = true;
    lidar_tf_sample_times_.push_back(now);
    current_lidar_tf_rate_hz(now);
}

double LidarNavControl::current_lidar_tf_rate_hz(const rclcpp::Time& now) {
    const double window_s = std::max(lidar_tf_rate_window_s_, 1e-3);
    while (!lidar_tf_sample_times_.empty() &&
           (now - lidar_tf_sample_times_.front()).seconds() > window_s) {
        lidar_tf_sample_times_.pop_front();
    }
    if (lidar_tf_sample_times_.size() < 2) {
        return 0.0;
    }
    const double span_s =
        (lidar_tf_sample_times_.back() - lidar_tf_sample_times_.front()).seconds();
    return static_cast<double>(lidar_tf_sample_times_.size() - 1) /
           std::max(span_s, 1e-3);
}

bool LidarNavControl::lidar_tf_rate_guard_allows_control(
    const rclcpp::Time& now, double& rate_hz, double& age_s, std::string& reason)
{
    rate_hz = current_lidar_tf_rate_hz(now);
    age_s = last_lidar_tf_time_valid_ ? (now - last_lidar_tf_time_).seconds() : 0.0;
    reason.clear();

    if (!lidar_tf_rate_guard_enable_ || !got_tf_ || !last_lidar_tf_time_valid_) {
        if (lidar_tf_rate_guard_active_) {
            RCLCPP_INFO(this->get_logger(), "Lidar TF rate guard disabled or no TF state; resuming control.");
        }
    ...
    if (age_s > lidar_tf_timeout_s_) {
        reason = "timeout";
    } else if (rate_hz < lidar_tf_min_rate_hz_) {
        reason = "low_rate";
    }
    ...
    if (reason.empty()) {
        if (lidar_tf_rate_guard_active_) {
            RCLCPP_INFO(this->get_logger(),
                        "Lidar TF rate recovered: rate=%.2f Hz age=%.3f s; resuming cmd_vel.",
                        rate_hz, age_s);
        }
        lidar_tf_rate_guard_active_ = false;
        lidar_tf_guard_stop_sent_ = false;
...
bool LidarNavControl::lidar_pose_guard_allows_control(std::string& reason) {
    reason.clear();

    if (!lidar_pose_guard_enable_ || !got_tf_) {
        if (lidar_pose_guard_active_) {
            RCLCPP_INFO(this->get_logger(), "Lidar pose guard disabled or no TF state; resuming control.");
        }
    ...
    const bool out_of_bounds =
        x_ < lidar_pose_min_x_ || x_ > lidar_pose_max_x_ ||
        y_ < lidar_pose_min_y_ || y_ > lidar_pose_max_y_;
    if (!out_of_bounds) {
        if (lidar_pose_guard_active_) {
            RCLCPP_INFO(this->get_logger(),
                        "Lidar pose recovered: pose(%.2f, %.2f) within [x:%.1f,%.1f] [y:%.1f,%.1f]; "
                        "resuming cmd_vel.",
                        x_, y_,
                        lidar_pose_min_x_, lidar_pose_max_x_,
                        lidar_pose_min_y_, lidar_pose_max_y_);
        }
        lidar_pose_guard_active_ = false;
        return true;
    }
...
void LidarNavControl::reset_lidar_guard_control_state() {
...
void LidarNavControl::publish_lidar_guard_stop() {
    reset_lidar_guard_control_state();
...
bool LidarNavControl::handle_pre_path_spin() {
    ...
    if (pre_scan_spin_done_time_valid_ &&
        !ocr_result_locked_ &&
        !ocr_audio_played_ &&
        (now - pre_scan_spin_done_t0_).seconds() >= pre_scan_ocr_audio_timeout_sec_) {
        RCLCPP_WARN(this->get_logger(),
                    "No OCR %.1f s after pre-scan spin done; playing fallback audio 0.wav.",
                    pre_scan_ocr_audio_timeout_sec_);
    ...
    if ((!pre_path_spin_enable_ || pre_path_spin_target_yaws_.empty()) && pre_scan_pause_after_done_) {
        if (!pre_scan_spin_done_) {
            ...
            pre_scan_spin_done_t0_ = now;
            pre_scan_spin_done_time_valid_ = true;
            RCLCPP_INFO(this->get_logger(),
                        "Pre-scan disabled/empty. Waiting for remote start before record_pass first point.");
            if (!pre_scan_spin_event_sent_) {
                publish_logistics_event(LogisticsEventType::PRE_SCAN_SPIN_DONE, current_wp_idx_);
                pre_scan_spin_event_sent_ = true;
            }
    ...
    if (pre_scan_fallback_enable_ &&
        ...
        pre_scan_spin_done_time_valid_ = true;
        RCLCPP_WARN(this->get_logger(),
                    "Pre-scan timeout after %.1f s. Waiting for remote start before record_pass first point.",
                    pre_scan_timeout_s_);
        if (!pre_scan_spin_event_sent_) {
            publish_logistics_event(LogisticsEventType::PRE_SCAN_SPIN_DONE, current_wp_idx_);
            pre_scan_spin_event_sent_ = true;
        ...
        case PrePathSpinPhase::SeekYaw: {
            ...
            if (std::abs(err_yaw) < pre_path_spin_yaw_tol_) {
                ...
                if (pre_path_spin_yaw_idx_ + 1 >= pre_path_spin_target_yaws_.size()) {
                    ...
                    pre_scan_spin_done_time_valid_ = true;
                    RCLCPP_INFO(this->get_logger(),
                                "Pre-scan spin done. Waiting for remote start before record_pass first point.");
                    if (!pre_scan_spin_event_sent_) {
                        publish_logistics_event(LogisticsEventType::PRE_SCAN_SPIN_DONE, current_wp_idx_);
                        pre_scan_spin_event_sent_ = true;
                    }
...
void LidarNavControl::control_loop() {
    ...
    double lidar_tf_rate_hz = 0.0;
    double lidar_tf_age_s = 0.0;
    std::string lidar_tf_guard_reason;
    if (!lidar_tf_rate_guard_allows_control(
            this->get_clock()->now(), lidar_tf_rate_hz, lidar_tf_age_s,
            lidar_tf_guard_reason)) {
        publish_lidar_guard_stop();
        lidar_tf_guard_stop_sent_ = true;
        lidar_tf_rate_guard_active_ = true;
        RCLCPP_WARN_THROTTLE(
            this->get_logger(), *this->get_clock(), 2000,
            "Lidar TF rate guard active (%s): rate=%.2f Hz < %.2f Hz or age=%.3f s > %.3f s; "
            "publishing continuous zero cmd_vel and paused motion control.",
            lidar_tf_guard_reason.c_str(),
            lidar_tf_rate_hz, lidar_tf_min_rate_hz_,
            lidar_tf_age_s, lidar_tf_timeout_s_);
        return;
    }

    std::string lidar_pose_guard_reason;
    if (!lidar_pose_guard_allows_control(lidar_pose_guard_reason)) {
        publish_lidar_guard_stop();
        lidar_pose_guard_active_ = true;
        RCLCPP_WARN_THROTTLE(
            this->get_logger(), *this->get_clock(), 2000,
            "Lidar pose guard active (%s): 雷达飘了! pose(%.2f, %.2f) out of bounds "
            "[x:%.1f,%.1f] [y:%.1f,%.1f]; publishing continuous zero cmd_vel.",
            lidar_pose_guard_reason.c_str(), x_, y_,
            lidar_pose_min_x_, lidar_pose_max_x_,
            lidar_pose_min_y_, lidar_pose_max_y_);
        return;
    }
    ...
    const bool pass_through_cross_band_transition =
        current_wp.type == WaypointType::TRANSITION &&
        current_wp.cross_isolation_band &&
        !current_wp.direct_nearest_approach &&
        !current_wp.manip_approach_arrival &&
        !current_wp.post_manip_exit;
    if (pass_through_cross_band_transition &&
        !current_wp.corridor_lateral_only_arrival) {
        dist_tol = cross_band_pass_through_dist_arrival_tolerance_;
    }
    ...
    if (update_arrival_debounce(
        ...
        if (pass_through_cross_band_transition) {
            ...
            RCLCPP_INFO(
                this->get_logger(),
                "Reached cross-band pass-through TRANSITION point. Passing to next without stop.");
            std_msgs::msg::Bool status_msg;
            status_msg.data = true;
            nav_status_pub_->publish(status_msg);
            return;
    ...
    const bool pickup_dock_stage =
        current_wp.type == WaypointType::PICKUP &&
        (controller_output.stage == NavControlStage::DOCK_YAW ||
         controller_output.stage == NavControlStage::DOCK);
    if (pickup_dock_stage &&
        (!pickup_dock_event_sent_ || pickup_dock_event_wp_idx_ != current_wp_idx_)) {
        publish_logistics_event(LogisticsEventType::PICKUP_DOCK_ENTERED, current_wp_idx_);
        pickup_dock_event_sent_ = true;
        pickup_dock_event_wp_idx_ = current_wp_idx_;
        RCLCPP_INFO(this->get_logger(),
                    "WP[%zu] PICKUP dock entered. Published logistics event PICKUP_DOCK_ENTERED.",
                    current_wp_idx_);
    }
