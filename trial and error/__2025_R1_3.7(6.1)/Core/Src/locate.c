...
备注：    1.此函数中的运算都是在弧度为单位的情况下进行的
				  2.该函数是为了初始化随动轮
*******************************************************************************************/
//void DT35_Location_Init(ST_ROBOT *pstRobot, ST_FOLLOWER_WHEEL *pstFW, ST_DT35 *p_dt35_save, ST_DT35 *p_dt35_now)
//{
//    fp32 fpQ;
//    p_dt35_now->robot_q = pstRobot->stPos.fpPosQ;//单位rad
...
//    p_dt35_now->robot_x = (p_dt35_now->dt35_x2 + DT35_X2) * cos(p_dt35_now->robot_q) ;
//    p_dt35_now->robot_y = (p_dt35_now->dt35_y2 + DT35_Y2) * cos(p_dt35_now->robot_q) ;
...
void DT35_relocation_new(ST_ROBOT *pstRobot, ST_FOLLOWER_WHEEL *pstFW, ST_DT35 *p_dt35_save, ST_DT35 *p_dt35_now)
{
    ...
    p_dt35_now->robot_y = (p_dt35_now->dt35_y1) * cos(fpQ);
...
/*-------------------------------------------------------------------------------------------------
函数功能：将MID360发过来的位置数据转换成增量式，后续使用DT35进行刷新
-------------------------------------------------------------------------------------------------*/
void Mid_360_location(ST_ROBOT *pstRobot)
{
	radar_x_td.aim = Vision_Data.radar_x;
	radar_y_td.aim = Vision_Data.radar_y;
	
	CalTD(&radar_x_td);
	CalTD(&radar_y_td);
	
	radar_x_lpf.in = radar_x_td.x2;
	radar_y_lpf.in = radar_y_td.x2;
	
	LpFilter(&radar_x_lpf);
	LpFilter(&radar_y_lpf);
	
	pstRobot->stPos.fpPosX += radar_x_lpf.out*0.02f;
	pstRobot->stPos.fpPosY += radar_y_lpf.out*0.02f;
...
void dt35_relocation(void)
{
	...
	{
		if(DT35_NEW.dt35_x1>8000||DT35_NEW.dt35_x1<50
		 ||DT35_NEW.dt35_x2>8000||DT35_NEW.dt35_x2<50)
			DT35_NEW.x_sucpect = 1;
		if(DT35_NEW.dt35_y1>9500||DT35_NEW.dt35_y1<50
		 ||DT35_NEW.dt35_y2>9500||DT35_NEW.dt35_y2<50)
			DT35_NEW.y_sucpect = 1;
	...
	{
		if(DT35_NEW.dt35_x1>9500||DT35_NEW.dt35_x1<50
		 ||DT35_NEW.dt35_x2>9500||DT35_NEW.dt35_x2<50)
			DT35_NEW.x_sucpect = 1;
		if(DT35_NEW.dt35_y1>8000||DT35_NEW.dt35_y1<50
		 ||DT35_NEW.dt35_y2>8000||DT35_NEW.dt35_y2<50)
			DT35_NEW.y_sucpect = 1;
