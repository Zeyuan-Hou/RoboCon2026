...
typedef struct
{
	...
	fp32 t1;//减速时间
	ST_VEL Accup;//加速加速度
	ST_VEL Accdown;//减速加速度
	ST_VEL VIN;//开始减速时的初速度
...
extern uint16_t ClipShort(uint16_t usValue, uint16_t usMin, uint16_t usMax);
...
extern void CalT_dST(T_fasedown * T_dST,stPoint Start,stPoint End,ST_VEL Vin,fp32 A);
